...
async function list(req, res) {
  try {
    const [services] = await db.query(
      'SELECT id, user_id, name, link_id, min_advance_hours, created_at FROM services WHERE user_id = ? ORDER BY created_at ASC',
      [req.user.id]
    );
    const result = [];
    for (const svc of services) {
      ...
      result.push({
        ...
        name:             svc.name,
        linkId:           svc.link_id,
        minAdvanceHours:  svc.min_advance_hours,
...
async function create(req, res) {
  try {
    ...
    const [result] = await db.query(
      'INSERT INTO services (user_id, name, link_id, min_advance_hours) VALUES (?, ?, ?, ?)',
      [req.user.id, name.trim(), linkId, 24]
    );
...
async function update(req, res) {
  try {
    const { name, minAdvanceHours } = req.body;
    if (!name?.trim()) return res.status(400).json({ error: 'El nombre es requerido' });
    const [result] = await db.query(
      'UPDATE services SET name = ?, min_advance_hours = ? WHERE id = ? AND user_id = ?',
      [name.trim(), minAdvanceHours || 24, req.params.id, req.user.id]
    );
